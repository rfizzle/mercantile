...
- [REPO-LAYOUT.md](https://github.com/rfizzle/concord/blob/master/REPO-LAYOUT.md) — where non-code files live (migrated 2026-06-12)
...
second manifest declaring the dev-only `mercantile-gametest` mod — never in the
...
- **Mod ID:** `mercantile` — use `Mercantile.id("path")` to create
...
- **Asset philosophy:** Vanilla-first *where a vanilla asset maps cleanly* —
  ...
  lock/unlock glyphs, the reputation HUD balance-scale glyph, the profession-tinted
...
- **Commits:** [Conventional Commits](https://www.conventionalcommits.org/)
  with a topical scope naming the feature area: `feat(trade): …`,
  `fix(villager): …`, `refactor(sentry-pylon): …`, `ci(review): …`,
  ...
  partial work).
  Note: historical commits use a `S-NNN` scope tied to a sprint-tracking
  file that no longer exists — do not emulate that pattern in new commits.

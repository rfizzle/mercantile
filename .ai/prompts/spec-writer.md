You are writing an implementation spec for a GitHub issue on the Mercantile
...
3. **`SPEC.md`** — full feature spec for the mod. Cross-reference whether the
   issue is already described there.
...
- `src/main/resources/mercantile.mixins.json` — <if a new mixin is added>
- <…>

### Skills to consult
- `.ai/skills/mc-<area>/SKILL.md` — <why>
- <…>
...
- **Respect project conventions** documented in `AGENTS.md`: Mojang mappings,
  `Mercantile.id()` for ResourceLocations, vanilla-first assets, correct
  source set placement, Conventional Commits with topical scope.

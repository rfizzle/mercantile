...
These are not user-facing features but are required by everything that follows.
...
6. **Villager Names** (section 2) — Data-driven name pools, spawn assignment mixin, nametag priority logic.
7. **Profession Lock Protection** (section 12) — Single mixin, simple NBT flag.
8. **Villager Healing Enhancement** (section 13) — Single mixin, configurable multiplier.
9. **Villager Sound Volume** (section 16) — Client-only mixin, config slider.
10. **Pathfinding Improvements** (section 6) — Four independent mixins, individually toggleable.

### Phase 2: Trade GUI Features
These all mixin into the merchant screen and share UI patterns. Develop in this order to avoid mixin conflicts.

11. **Bulk Trading** (section 8) — Server-side `MerchantMenu` mixin.
12. **Trade Restock Indicator** (section 9) — First feature using the trade-screen custom payload pattern. Establishes the S2C data channel for trade GUI metadata.
13. **Demand Pricing Transparency** (section 10) — Builds on the same payload channel from #12.
14. **Villager Info Panel** (section 14) — Extends the trade screen with a side panel. Uses payload channel.

### Phase 3: Villager Pickup
Depends on nothing, but is complex and benefits from having names (phase 1) and profession support infra (phase 0) already working so the pickup item displays complete data.

15. **Villager Pickup** (section 1) — NBT capture/restore, player head item with minecraft-heads.com textures, tooltip rendering, placement logic, raid restriction, multiplayer guards.

### Phase 4: Trade Cycling + Reputation
Trade cycling is standalone but reputation builds on it (cycling grants rep). Reputation then unlocks exclusive trades which affect the trade GUI (phase 2 patterns).

16. **Trade Cycling** (section 3) — Cycle button in merchant screen, C2S packet, server re-roll logic, locked trade tracking.
17. **Reputation System** (section 4) — Per-player score, action hooks, tier effects, price modifiers, exclusive trade injection, datapack loading.

### Phase 5: Villager AI
AI-related features that modify villager behavior beyond trading.

18. **Villager Follow Mode** (section 5) — Custom AI goal, follow state, collision immunity, particle indicator.

### Phase 6: Visualization
Client-side rendering features. All share particle/render infrastructure.
